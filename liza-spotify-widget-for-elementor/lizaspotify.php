...
if ( function_exists( 'liza_spotify_fs' ) ) {
    liza_spotify_fs()->set_basename( false, __FILE__ );
} else {
    /**
    ...
    if ( !function_exists( 'liza_spotify_fs' ) ) {
        ...
        class LizaSpotify {
            ...
            public function add_elementor_widget_category( $elements_manager ) {
                $elements_manager->add_category( 'liza-spotify', [
            ...
            public function elementor_missing_notice() {
                ...
                $message = sprintf( esc_html__( '"%1$s" requires "%2$s" to be installed and activated.', 'liza-spotify' ), '<strong>' . esc_html__( 'Liza Spotify Widgets Pro', 'liza-spotify' ) . '</strong>', '<strong>' . esc_html__( 'Elementor', 'liza-spotify' ) . '</strong>' );
                ...
                <div class="notice ruthless-promo-notice is-dismissible">
                    <div class="ruthless-promo-content">
                        ...
                        <div class="ruthless-promo-text">
                            <h3><?php 
                _e( 'Enhance Your Elementor Website with Custom Fonts!', 'liza-spotify' );
                ?></h3>
                            <p><?php 
                _e( 'Take your design to the next level with ', 'liza-spotify' );
                ?>
                            <a href="https://www.ruthlesswp.com/plugins/ruthless-custom-fonts-for-elementor" target="_blank">
                                <?php 
                _e( 'Ruthless Custom Fonts for Elementor', 'liza-spotify' );
                ?>
                            </a>
                            <?php 
                _e( ' - Upload and use any custom font in your Elementor designs.', 'liza-spotify' );
                ?></p>
                        </div>
                        <a href="https://www.ruthlesswp.com/plugins/ruthless-custom-fonts-for-elementor" class="button button-primary" target="_blank">
                            <?php 
                _e( 'Learn More', 'liza-spotify' );
                ?>
                        </a>
                    </div>
                </div>
                    ...
                    $(document).on('click', '.ruthless-promo-notice .notice-dismiss', function() {
                        $.ajax({
                            ...
                            data: {
                                ...
                                nonce: '<?php 
                echo wp_create_nonce( 'dismiss_ruthless_promo' );
                ?>'
                            }
                        });
                    });
